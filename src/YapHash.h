...
public:
    YapHash(const Audio& wave, MelFb& melFb, Parameter *pParam);
    ~YapHash();

    unsigned int length() const {return indexLen;}
    //    int* index() const {return index;}
    unsigned long *index; // hashtupels (hash + position)

private:
    int hashLen;
    unsigned long *hash;
    unsigned int indexLen; // number of tupels (hash + position)
