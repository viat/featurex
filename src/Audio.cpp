...
Audio::Audio(short *waveSamples, int waveLength, Parameter *param) //int samplerate, int thresholdVAD, int thresholdDeclicker, int preEmphasizeFactor)
{
	len = waveLength;
	fs = param->samplerate;
	len = MIN(param->maxAudioLen*param->samplerate, waveLength);// max seconds of audio

	// convert to float and divide by 2^15
	Fw32f *tmpWav = fwsMalloc_32f(len);
	fwsConvert_16s32f_Sfs(waveSamples, tmpWav, len, 15);

	// declicker reduces short spikes in audio data
	deClicker(tmpWav, len, param->declickerThreshold);

	// pre emphasize audio
	preEmphasize(tmpWav, len, (param->preEmphasizeFactor/100.0));
	...
	cutACFVad_I(tmpWav, &len, param->vadThreshold);
