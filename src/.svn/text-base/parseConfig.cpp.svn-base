...
int parseConfig(const char *configfile, Parameter *pParam) {
	...
	while (fgets(line, 256, conffile) != NULL) {
		...
		else if (strcmp(optionname, "INDEXD_PORT") == 0)
			pParam->indexdPort = (unsigned int) atoi(optionvalue);

		else if (strcmp(optionname, "CALLX_PORT") == 0)
			pParam->callxPort = (unsigned int) atoi(optionvalue);
		...
		else if (strcmp(optionname, "WRITE_TO_SOCKET") == 0)
			pParam->writeToSocket = (unsigned int) atoi(optionvalue);
	}
