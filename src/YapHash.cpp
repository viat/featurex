...
YapHash::YapHash(const Audio& audio,  MelFb& melBank, Parameter *param)
{
    int maxHashLen = getNoOfWindows(audio.length(), param->windowSize, param->stepSize) - 1;
    hash = (unsigned long*) malloc(sizeof(unsigned long) * maxHashLen);
...
void YapHash::calcHash(const Audio& audio, MelFb& melBank, Parameter *param)
{
    ...
    indexLen = k;
    cout << index[indexLen*2-2] << ", " << index[indexLen*2 -1] << endl;
    ...
    if (param->debugLevel > 2)
    	writeIndexToCSV("debugHash.csv", index, indexLen/2);
}
